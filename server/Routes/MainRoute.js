...
mainRouter.get(
  ...
  handle_errors(async (req, res) => {
    ...
    if (result.includes("Invalid")) res.status(400).send("Invalid Query");
    else if (result.length == 0) res.status(404).send("Invalid user_id");
...
mainRouter.get(
  ...
  handle_errors(async (req, res) => {
    ...
    if (result.length == 0) res.status(404).send("No Words of the Day");
    else res.json(result);
  })
);
...
mainRouter.post(
  ...
  handle_errors(async (req, res) => {
    ...
    if (userCount[0].count == 0) res.status(400).send("User Does Not Exist");
      ...
      if (remainingUserScores[0].calc_max_remaining_scores <= 0)
        ...
        if (result.includes("not-null")) res.send("Success");
